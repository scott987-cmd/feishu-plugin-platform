...
- **客户端内嵌的是只读 token(`PLATFORM_READ_TOKEN`)** → 即便被提取也只能读(`GET /api/apps*` + `/api/execute`);写/删/生成需服务端持有的 admin token 或会话。要在 Bitable webview 内做真·per-user 身份,升级到飞书 webview-OAuth(见 `docs/PRODUCTION.md` §7)。

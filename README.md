...
> 🔗 **在线介绍页**：<https://scott987-cmd.github.io/feishu-plugin-platform/>
...
打开 <http://localhost:8080/shortcut.html> —— 在 **字段捷径 / 自动化** 之间切换，输入一个请求，点击 一键生成，审阅**可审计的源码**，并下载工程 `.zip`（或只下载 DSL `.json`）。字段捷径还可在 UI 内**试运行**：用样例输入把生成的 DSL POST 到内联 `/api/execute`，上传送审前先真跑一遍。
...
- **能力分离鉴权（B1）。** 客户端 bundle 只内嵌**只读** `PLATFORM_READ_TOKEN`（`GET /api/apps*` + `POST /api/execute`）；写/管理操作（`POST`/`DELETE /api/apps`、`/api/generate`）要求仅存于服务端的 admin `PLATFORM_API_TOKEN`；`/api/me` 与 `/api/my/*` 走会话。即便客户端 token 泄露也只能读（IDOR 被消除），`path.Clean` 防 `//api` 绕过。（`internal/api/server.go`、`cmd/api/main.go`）
- **审计 + 出网账本。** `GET /api/audit`（仅 admin，最新优先）暴露一条 append-only 审计流，落库到一张专用 `audit_log` 多维表格（`FEISHU_AUDIT_TABLE_ID`，留空=仅 stdout；落库失败绝不让请求失败）。每一次出网调用都会在同一张表里记一条 `action=execute.egress`（SSRF/重定向拦截记为 `outcome=error`），记录点在 `execrt.fetch` 收口处，热路径安全（stdout 恒发 + 异步缓冲，关机时优雅 drain）。（`internal/store/audit_bitable.go`、`internal/execrt/engine.go`）
...
- ✅ **生产部署已端到端验证（2026-06-25）：** 平台以单节点 docker compose + Caddy 自动 TLS（Let's Encrypt，经 `<ip>.sslip.io` 魔法 DNS host）跑在一台 AWS EC2 上，`STORE=bitable`。生产 compose 是 `deploy/compose/docker-compose.prod.yml`；仓库根的 `docker-compose.yml` 仅供本地开发（`STORE=memory`、`CORS=*`）。k8s（`deploy/k8s/`）是**可选**的横向扩展路径，非主线。部署步骤、必填 token 与 `ALLOWED_ORIGIN` 坑见 **[`deploy/compose/DEPLOY.md`](deploy/compose/DEPLOY.md)**（`GENERATOR_TOKEN`/`EXECUTE_RUNNER_TOKEN`/`PLATFORM_READ_TOKEN` 为 compose 必填；auth 开启时 `ALLOWED_ORIGIN=*` 会让 api 拒绝启动）。
- ✅ **人类可读的错误：** `generator.Explain` 把 TS2554 / 域名白名单 / 主列类型 / 缺字段 / 模型+网络 / 修复耗尽等映射成朴素提示（`{error:hint, detail:raw}`），UI 展示提示 + 可折叠原文。
- ✅ **AI 出网可控：** `AI_ENABLED` 总开关、`DEEPSEEK_BASE_URL` 端点固定、启动时出网透明日志；容灾备份=飞书 Base 副本 + `scripts/backup-defs.sh`。

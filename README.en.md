...
> 🔗 **Live overview page**: <https://scott987-cmd.github.io/feishu-plugin-platform/>
...
Open <http://localhost:8080/shortcut.html> — toggle between **字段捷径 / 自动化**, type a request, click 一键生成, review the **auditable source**, and download the project `.zip` (or just the DSL `.json`). Field shortcuts also get an in-UI **dry-run**: it POSTs the generated DSL with sample inputs to the inline `/api/execute`, so you can really run it once before uploading for review.
...
- **Capability-split auth (B1).** The client bundle embeds only the **read-only** `PLATFORM_READ_TOKEN` (`GET /api/apps*` + `POST /api/execute`); writes/management (`POST`/`DELETE /api/apps`, `/api/generate`) require the server-only admin `PLATFORM_API_TOKEN`; `/api/me` and `/api/my/*` use the session. A leaked client token can only read (IDOR eliminated), and `path.Clean` guards against `//api` evasion. (`internal/api/server.go`, `cmd/api/main.go`)
- **Audit + egress ledger.** `GET /api/audit` (admin-only, newest-first) exposes an append-only audit trail persisted into a dedicated `audit_log` Bitable table (`FEISHU_AUDIT_TABLE_ID`, empty = stdout-only; an append failure never fails the request). Every outbound call also records `action=execute.egress` into the same table (SSRF/redirect blocks = `outcome=error`), captured at the `execrt.fetch` choke point and hot-path-safe (stdout always + async-buffered, graceful drain on shutdown). (`internal/store/audit_bitable.go`, `internal/execrt/engine.go`)
...
- ✅ **Production deploy verified end-to-end (2026-06-25):** the platform runs as a single-node docker compose + Caddy auto-TLS (Let's Encrypt via an `<ip>.sslip.io` magic-DNS host) on an AWS EC2 host, `STORE=bitable`. The prod compose is `deploy/compose/docker-compose.prod.yml`; the repo-root `docker-compose.yml` is local-dev only (`STORE=memory`, `CORS=*`). k8s (`deploy/k8s/`) is the **optional** scale-out path, not the primary. For deploy steps, required tokens, and the `ALLOWED_ORIGIN` gotcha see **[`deploy/compose/DEPLOY.en.md`](deploy/compose/DEPLOY.en.md)** (`GENERATOR_TOKEN`/`EXECUTE_RUNNER_TOKEN`/`PLATFORM_READ_TOKEN` are required by compose; with auth enabled `ALLOWED_ORIGIN=*` makes the api refuse to start).
- ✅ **Human-readable errors:** `generator.Explain` maps TS2554 / domain-allowlist / primary-column-type / missing-field / model+network / repair-exhaustion to a plain hint (`{error:hint, detail:raw}`), and the UI shows the hint + a collapsible raw detail.
- ✅ **AI egress controls:** an `AI_ENABLED` kill-switch, `DEEPSEEK_BASE_URL` endpoint pinning, and a boot-time egress-transparency log; DR/backup = a Feishu Base copy + `scripts/backup-defs.sh`.

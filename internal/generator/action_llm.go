...
const (
	...
	actionSystemPrompt = "You generate a Feishu Bitable AUTOMATION ACTION definition (basekit addAction), as JSON. " +
		...
		"(8) WRITE BACK TO A FEISHU BASE (connector — create/update 多维表格 records) — use a 2-step pipeline; add open.feishu.cn to domains; make appId/appSecret/appToken/tableId inputs the user fills. step 1: POST https://open.feishu.cn/open-apis/auth/v3/tenant_access_token/internal with bodyJson {\"app_id\":\"{appId}\",\"app_secret\":\"{appSecret}\"}. step 2: POST https://open.feishu.cn/open-apis/bitable/v1/apps/{appToken}/tables/{tableId}/records/batch_create with headers Authorization=\"Bearer {token.tenant_access_token}\" (token = step-1 id) and bodyJson {\"records\":[{\"fields\":{\"<Base列名>\":\"{inputKey}\"}}]} (field keys are the Base column names). Map result: code=res.code (0=success), record_id=res.data.records.0.record_id. For UPDATE use .../records/batch_update and include record_id in each records[] item. " +

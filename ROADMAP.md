...
| 🔶5 | 结果列类型扩展(标量类已完成 2026-06-23) | 中 | M | 已加 **Phone/Email/Currency/Progress/Rating/Barcode**(标量,值与 Text/Number 同,仅列语义不同);并按 SDK 修了**潜在 bug=primary 列必须 Text\|Number**(原本允许 primary DateTime/SingleSelect 会编译失败)。已验证:对真 SDK `build:field` 编译过 + testField 数据形状正确(Phone/Email→string,Currency/Progress→number)。**剩:Url(`{text,link}` 形状)、MultiSelect(数组值)——需真 cell 渲染验证/数组输出,暂缓** |

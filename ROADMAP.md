...
| 🔶4 | **写路径栈**:✅action bodyJson + ✅`PUT/PATCH/DELETE`+✅自定义header(2026-06-23)→ ⬜多步链式 | 高/中 | M→S→L | 已完成:`ValidMethods` 加 PUT/PATCH/DELETE;`Execute.Headers`(name→value,`{key}`可注入,Content-Type/auth 自动加且优先);body/bodyJson 适用 POST/PUT/PATCH;统一 `renderFetchInit`(field/action 共用,action 重绑 inp.→args.)。真机:httpbin PUT(field·testField method_seen=PUT)、PATCH(action·testAction method_seen=PATCH)+自定义头+bodyJson 全回显正确。**剩:多步链式调用(L,Execute→steps[],仍是连接器最后一块技术前置)** |
...
| 🎯 | **连接器 / 写回 Base 记录** | 高 | L | 最大诉求+最大架构缺口:两 surface 都只能写回触发单元格,**无写任意记录/建行/跨表的 DSL 构造**。要么 action 调 OpenAPI(`batch_create/update`+`bitable:app`,触发权限重审),要么开 client-side 视图轨。**战略大注,非快赢** |

...
| ✅5 | 结果列类型扩展(标量 + Url 已完成 2026-06-23) | 中 | M | 已加 **Phone/Email/Currency/Progress/Rating/Barcode**(标量)+ **Url 可点击链接**(渲染成 `{text,link}` cell 值);并按 SDK 修了 **primary 列必须 Text\|Number**。已验证:`build:field` 对真 SDK 编译过、testField 形状正确;**Url 的 `{text,link}` 形状对真 Base URL 字段写读确认**(write code:0,读回 `{link,text}`)。plugin-center 加 `github-homepage`(用户名→可点击主页)。**剩:MultiSelect(数组值,需数组输出)** |

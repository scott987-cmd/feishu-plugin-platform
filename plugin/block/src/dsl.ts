...
/** 组件类型 — Go: ValidComponents。gauge=进度环,pivot=透视表,timeline=时间轴,
...
export interface Component {
  ...
  y?: AggSpec;
  text?: string;
  /** TABLE only: 要展示的列(字段名);省略 = 全部字段。 */
  columns?: string[];
  /** GAUGE only: 目标值(常量);进度 = value / target。 */
  target?: number;
  /** PIVOT only: 列分组字段(行分组用 x)。 */
  col?: string;
  /** CHART only: 对分组结果排序(配合 limit 做 TopN/排行榜)。 */
  sort?: "asc" | "desc";

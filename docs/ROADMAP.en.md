...
> Production method: 6-way parallel web research on the Feishu plugin ecosystem → graded against this platform's "pre-reviewed container + read-only data DSL" model → adversarial review and finalization (2026-06-22; yellow-zone shipping progress updated 2026-06-25).
> After deduplication, roughly **30 categories**: 🟢 5 / 🟡 15 / 🔴 12 (several yellow-zone items have shipped — the container's two-layer abstraction, detail table + a real filter, gauge, markdown, pivot, timeline, kanban/calendar/gallery, countdown renderers are all live in the `renderers` registry of `plugin/block/src/App.tsx`; still yellow = tree / map / record view / advanced-chart family / automation-lite).
...
> Progress (2026-06-25 re-audit of `plugin/block/src`): cells marked ✅ are live — the two-layer abstraction (`ops.ts` operator registry + the `renderers` registry in `App.tsx`) + a real filter (`filter.ts` `applyFilter` predicate) shipped, and the gauge/markdown/pivot/timeline/kanban/calendar/gallery/countdown renderers are all registered. Still yellow = tree / map / record view / advanced-chart family / automation-lite.

| Type | DSL building block to add | Size | Notes |
|---|---|---|---|
| ✅ **Container interpreter / rendering layer (foundation)** | Two layers of "aggregation operator + renderer" (register pattern); cross-table read-only data access layer | M | **The real asset — shipped.** `ops.ts` is the operator registry (the `aggregators` table; adding a statistic = registering a function), `App.tsx`'s `renderers` is the renderer registry (adding a chart = registering a renderer); the interpreter itself is reviewed only once |
| ✅ **Detail table (select columns + filter)** | table renderer; **truly implement the filter operator**; row-level read-only (note batch read ≤200) | M | **Shipped**: `filter.ts` `applyFilter` is a real predicate (hand-written parser, never eval; supports `= != > >= < <= contains in` + `AND/OR` + month/year/day + THIS_MONTH/THIS_YEAR/TODAY macros; parse failure = no filter + amber warning). The table renderer is registered and wired to the filter |
| ✅ Progress chart / goal attainment (gauge) | gauge/progress renderer (current + target) | S | **Shipped**: `GaugeTile` is registered (value = field aggregation, target = a goal constant, draws a progress ring) |
| ✅ Cell Markdown / long-text preview | markdown renderer + **strong sanitization** | M | **Shipped**: the `Markdown` renderer parses to React elements and **never uses innerHTML/dangerouslySetInnerHTML** (the data-channel XSS defense), closing this safety red-line cell |
| ✅ Pivot table / pivot chart | pivot building block (two grouping fields + one aggregation) + 2D grouping | M | **Shipped**: `pivot` (the 2D cross-aggregation in ops.ts) + `PivotTile` are registered; pure aggregation, no new atomic operator |
...
| ✅ Timeline | timeline renderer (date + event, read-only layout) | S | **Shipped**: `TimelineTile` is registered (laid out sorted by a date field) |
| Hierarchy / tree | tree renderer (parent / bidirectional link → build the tree) | M | Read-only display, controlled collapsing OK (renderer not yet registered) |
| ✅ Read-only native view mirror (kanban/calendar/gallery) | read-only kanban / calendar / gallery renderer | L | **Shipped**: the three read-only renderers `KanbanTile`/`CalendarTile`/`GalleryTile` are all registered — **explicitly "read-only presentation," never promise drag-and-drop write-back** (write-back drops it into the red zone) |
...
| ✅ Countdown | countdown renderer + interpreter-built-in controlled tick | S | **Shipped**: `CountdownTile` is registered; the per-second controlled tick is provided by the interpreter, not user JS |
...
2. **The first version strictly holds to a "read-only data dashboard"**: the green zone + low-cost yellow zone (filter/table/progress/timeline/pivot — **all shipped**) can absorb almost all of Feishu's "dashboard + read-only visualization" demand.
3. ~~**The highest-priority engineering item = turn filter from a "not executed" badge into a real operator**~~ → **done**: `filter.ts` `applyFilter` is now a real predicate (hand-written parser, never eval), unblocking the detail-table / filtered-dashboard prerequisite.
4. **Abstract the container into two layers (aggregation operator + renderer) — shipped** (`ops.ts` operator registry + the `renderers` registry in `App.tsx`): this turns the yellow-zone advanced charts from "added one by one linearly" into "add a building block, reviewed only once." This is the engineering realization of the "bypass case-by-case review" insight.
...
1. ~~**Publish audit ledger**~~ → **Catalog write/delete audit + egress ledger are both done**: a persisted Bitable ledger + `GET /api/audit` (admin, append-only, newest-first); the egress ledger (per-call execute-runtime egress, `action=execute.egress`, recording host/method/outcome, SSRF/redirect blocks = error) is recorded at the `execrt.fetch` choke point, see [PRODUCTION](PRODUCTION.en.md) §11. **Still pending**: bundle↔reviewed-source hashing (attestation), see the wrap-up item in "Enterprise enhancements" #2 below.
...
| ✅2 | **Execute-runtime per-call egress ledger** — **done** | M | Records "which plugin sent which row's data to which external domain, for whom, allowed/blocked" — the DLP egress evidence a 信创 security team demands, backing the README's already-sold "egress audited in one place" | Shipped: an `EgressRecorder` interface records at the `fetch` choke point per hop (host/method/outcome/step; SSRF/redirect blocks = error) + `WithPluginID` attribution (attribution = the platform pluginId forwarded by api `/api/execute`, falling back to `fs.ID`); the runner's `egressRecorder` maps events to `execute.egress` audit records (actor=`plugin:<id>`) and appends them to the same audit table via an **async-buffered single worker** (hot-path-safe: stdout always + drops with a logged count when full, never slows execute). **Graceful drain**: SIGTERM → HTTP drains → the worker flushes its buffer before exit (10s cap), so a restart drops no buffered record. The runner needs `FEISHU_APP_ID/SECRET/BITABLE_APP_TOKEN` + `FEISHU_AUDIT_TABLE_ID` to persist. Tests: `internal/execrt/egress_test.go`. See PRODUCTION §11 |
...
> 🚀 **Live deployment (verified end-to-end 2026-06-25)**: production runs a single-node docker compose + Caddy auto-TLS on an AWS EC2 host (Caddy issues Let's Encrypt via an `<ip>.sslip.io` magic-DNS host; STORE=bitable); k8s (`deploy/k8s/`) is the optional future scale-out path, not the primary. Overview page: https://scott987-cmd.github.io/feishu-plugin-platform/.

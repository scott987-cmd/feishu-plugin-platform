...
1. **Publish audit ledger**: the publisher currently only takes screenshots for calibration, with no tamper-proof log of "who / when / which tenant / which version / source hash," and no **consistency comparison (attestation) between the uploaded bundle and the reviewed source**. → This is "Enterprise enhancements" #1/#2 below (persisted audit ledger + egress ledger).
...
|---|-----------|------|----------------|----------------------------------|
| 1 | **Persisted audit ledger in a Bitable + read-only viewer** | M | Turns the ephemeral `AUDIT` stdout line (lost on restart) into a filterable, tamper-resistant who/when/what/which-version trail — exactly "gap #1" above | Add an AuditStore that **replays the BitablePluginStore pattern** to append records to a dedicated Bitable table; replace the two `log.Printf("AUDIT")` calls in `server.go` with a store append; expose `GET /api/audit` (admin). Admins query it natively in the Base UI — no new console |
| 2 | **Execute-runtime per-call egress ledger** | M | Records "which plugin sent which row's data to which external domain, for whom, allowed/blocked" — the DLP egress evidence a 信创 security team demands, backing the README's already-sold "egress audited in one place" | The runner already centralizes every outbound call in `execrt.Engine.Run` and knows the host + PluginID; emit one structured record per fetch into the same audit table as #1. Highest compliance sensitivity in the platform |
| 3 | **In-UI dry-run ("试运行")** | M | A non-engineer sees the plugin call the real API and produce real values *before* the upload+review chain — kills the "generate a black box, upload blind, wait for review, find it was wrong" loop | execute-runner already runs a DSL+inputs and returns the mapped output; wire `web/shortcut.html` to POST the freshly-generated DSL + sample inputs through the existing inline `/api/execute`. Almost no backend work, biggest authoring-trust gain |
| 4 | **Human-readable failure explanations** | S | A non-engineer hitting a TS compiler stack-trace just gives up; friendly, actionable messages keep them in the flow and cut support load | The Verifier already feeds `build:field` compiler errors back into the repair loop; on final failure, add a translation layer (TS2554 "missing argument" → "the shortcut needs an input you didn't mention"). Lowest effort, broad reach |
...
> Recommendation: **do Tier 1 first (#1+#2+#3+#4)** — the audit/egress ledgers cement compliance and the "egress centralized" claim, dry-run + plain-language errors directly lift authoring UX, and all four are thin reuse with zero new dependencies. Pull Tier 2 by actual customer demand.

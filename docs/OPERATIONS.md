...
> **必填 token(prod compose 用 `${VAR:?}` 强制,缺则启动即失败)**:
> - `PLATFORM_API_TOKEN` —— admin/写 bearer,仅服务端持有(POST/DELETE /api/apps + /api/generate)
> - `PLATFORM_READ_TOKEN` —— 只读 token,会烤进客户端 bundle,**必须与 admin token 不同**(GET /api/apps* + POST /api/execute)
> - `GENERATOR_TOKEN` —— api↔generator 共享
> - `EXECUTE_RUNNER_TOKEN` —— api↔execute-runner 共享
>
> 每个都单独生成一个强随机值(**互不相同**):`openssl rand -hex 32`。
>
> **`ALLOWED_ORIGIN`**:启用了 auth 时**不能**用 `*`,否则 api 拒绝启动(`cmd/api/main.go` log.Fatal)。填具体来源(如 `https://<IP>.sslip.io`);仅本地 dev 调试可设 `ALLOWED_ORIGIN_INSECURE=true` 强行放行。
...
# GET /api/apps 用「只读」token(PLATFORM_READ_TOKEN)即可:
curl -s -H "Authorization: Bearer <PLATFORM_READ_TOKEN>" https://<DOMAIN>/api/apps   # JSON 列表
...
启用持久化审计后,平台把**管理操作**(action=…)与**每一次出网调用**(action=execute.egress,actor=plugin:<id>,含 method/outcome/step;SSRF/重定向被拦=outcome=error)追加到同一张 `audit_log` 表。

- 该表由 `cmd/bitable-bootstrap` 创建;运行后会打印 `FEISHU_AUDIT_TABLE_ID=…`,把它填进服务器 `.env`。
- `FEISHU_AUDIT_TABLE_ID` **留空 = 只写 stdout**(不落库),非空才持久化(落库失败永不让请求失败,best-effort)。
- 验证(`/api/audit` 只读 **admin** token,最新在前,`?limit≤1000`):

```bash
curl -s -H "Authorization: Bearer <PLATFORM_API_TOKEN>" \
  "https://<DOMAIN>/api/audit?limit=50"   # 最近 50 条审计/出网记录
# 未配置时返回 503:audit ledger not configured (set FEISHU_AUDIT_TABLE_ID)
```
...
| compose 报 `required variable … is missing a value` | 旧 `.env` 缺必填 token —— prod compose 用 `${VAR:?}` 强制;补齐 `PLATFORM_READ_TOKEN / GENERATOR_TOKEN / EXECUTE_RUNNER_TOKEN`(各 `openssl rand -hex 32`,互不相同)。 |
| api 反复重启 / 日志 `refusing to start: ALLOWED_ORIGIN=*` | 启用了 auth 却用了开放 CORS —— 把 `ALLOWED_ORIGIN` 设成具体来源,或仅 dev 设 `ALLOWED_ORIGIN_INSECURE=true`。 |
...
- `scripts/backup-defs.sh` —— 备份飞书 Base 里的应用/插件定义(DR/灾备)

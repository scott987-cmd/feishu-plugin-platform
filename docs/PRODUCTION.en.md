...
- **Egress ledger (`execute.egress`)** — for **every outbound call** (each hop of a multi-step pipeline) the execute-runner emits an `action=execute.egress` event (`actor=plugin:<id>`, `target=<external host>`, detail carries method/outcome/step): which plugin sent data to which external host, allowed or blocked (SSRF / redirect blocks land as `outcome=error`), into the **same audit table**. **Hot-path-safe**: execute is a high-frequency path, so recording goes to stdout (always) + an **async-buffered single worker** appends to Bitable (drops with a logged count when the buffer is full — it **never slows execute or hammers the Feishu QPS**). The runner persists only when `FEISHU_APP_ID/SECRET/BITABLE_APP_TOKEN` + `FEISHU_AUDIT_TABLE_ID` are set; otherwise egress is stdout-only.

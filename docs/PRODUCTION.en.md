...
- **Egress ledger (`execute.egress`)** — for **every outbound call** (each hop of a multi-step pipeline) the execute-runner emits an `action=execute.egress` event (`actor=plugin:<id>`, `target=<external host>`, detail carries method/outcome/step): which plugin sent data to which external host, allowed or blocked (SSRF / redirect blocks land as `outcome=error`), into the **same audit table**. Attribution uses the platform plugin id (forwarded by the api over `/api/execute`; it falls back to the shortcut's own id). **Hot-path-safe**: execute is a high-frequency path, so recording goes to stdout (always) + an **async-buffered single worker** appends to Bitable (drops with a logged count when the buffer is full — it **never slows execute or hammers the Feishu QPS**). **Graceful flush on shutdown**: on SIGTERM the HTTP server drains, then the worker writes every remaining buffered event before exiting (10s cap), so a rolling restart / redeploy **drops no buffered egress record**; stdout is always the backstop. The runner persists only when `FEISHU_APP_ID/SECRET/BITABLE_APP_TOKEN` + `FEISHU_AUDIT_TABLE_ID` are set; otherwise egress is stdout-only.

> This plus the egress ledger close the ROADMAP gap "Publish audit ledger" and "Enterprise enhancements #1/#2." Remaining increment: bundle↔reviewed-source hashing (attestation).
